...
type AdminTab = 'overview' | 'listings' | 'reviews' | 'claims';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        {tab === 'overview' && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              { label: 'Pending Listings', value: listings.length,           icon: <Clock className="w-5 h-5 text-amber-500" /> },
              { label: 'Pending Claims',   value: claims.length,             icon: <ShieldCheck className="w-5 h-5 text-blue-500" /> },
              { label: 'Total Reviews',    value: reviews.length,            icon: <MessageSquare className="w-5 h-5 text-purple-500" /> },
              { label: 'Reported Reviews', value: reviews.filter((r) => r.reportCount > 0).length, icon: <XCircle className="w-5 h-5 text-red-500" /> },

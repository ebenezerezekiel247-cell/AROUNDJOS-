...
import { MapPin, Star, Heart, Phone, MessageCircle, BadgeCheck, Flame, Crown } from 'lucide-react';
import { cn, getWhatsAppLink, getPhoneLink, formatNumber, getPriceSymbol } from '@/utils';
import { Badge, StarRating, VerifiedBadge } from '@/components/ui';
...
export function ListingCard({
  ...
  if (variant === 'horizontal') {
    return <HorizontalListingCard listing={listing} isFavorited={isFavorited} onFavorite={onFavorite} className={className} />;
  }
  if (variant === 'compact') {
    return <CompactListingCard listing={listing} className={className} />;
  }
  ...
  return (
    <div className={cn('group relative bg-white dark:bg-dark-card rounded-3xl overflow-hidden border border-surface-100 dark:border-dark-border shadow-card hover:shadow-card-hover transition-all duration-300 hover:-translate-y-1', className)}>
      ...
      <Link href={`/listing/${listing.slug}`} className="block relative overflow-hidden aspect-[4/3]">
        <Image
          src={listing.coverImage || '/placeholder.jpg'}
        ...
        {onFavorite && (
          <button
            ...
            className="absolute top-3 right-3 w-8 h-8 bg-white/90 dark:bg-black/50 backdrop-blur-sm rounded-full flex items-center justify-center shadow-sm hover:scale-110 transition-transform"
          >
        ...
        {listing.priceRange && (
          <div className="absolute bottom-3 left-3">
            <span className="bg-black/50 text-white text-xs font-medium px-2 py-0.5 rounded-lg backdrop-blur-sm">
              {getPriceSymbol(listing.priceRange)}
            </span>
          </div>
        )}
      </Link>
      ...
      <Link href={`/listing/${listing.slug}`} className="block p-4">
        ...
        <div className="flex items-start justify-between gap-2 mb-1">
          <h3 className="font-bold text-surface-900 dark:text-white text-sm leading-tight line-clamp-1 font-display">
        ...
        {listing.reviewCount > 0 && (
          <div className="flex items-center gap-1.5 mb-3">
            ...
            <span className="text-xs text-surface-400">({formatNumber(listing.reviewCount)})</span>
          </div>
        )}
        ...
        <div className="flex items-center gap-2 mt-3 pt-3 border-t border-surface-100 dark:border-dark-border">
          {listing.whatsapp && (
            <a
              ...
              className="flex-1 flex items-center justify-center gap-1.5 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 text-xs font-semibold py-2 rounded-xl hover:bg-emerald-100 dark:hover:bg-emerald-900/30 transition-colors"
            >
          ...
          {listing.phone && (
            <a
              ...
              onClick={(e) => e.stopPropagation()}
              className="flex-1 flex items-center justify-center gap-1.5 bg-brand-50 dark:bg-brand-900/20 text-brand-600 dark:text-brand-400 text-xs font-semibold py-2 rounded-xl hover:bg-brand-100 transition-colors"
            >
...
function HorizontalListingCard({
  listing,
  isFavorited,
  onFavorite,
  className,
    ...
    <div className={cn('group bg-white dark:bg-dark-card rounded-2xl overflow-hidden border border-surface-100 dark:border-dark-border flex', className)}>
      ...
      <Link href={`/listing/${listing.slug}`} className="flex-1 p-3 min-w-0">
        <div className="flex items-start justify-between gap-1 mb-1">
          ...
          {onFavorite && (
            <button onClick={(e) => { e.preventDefault(); onFavorite(); }} className="flex-shrink-0">
              <Heart className={cn('w-4 h-4', isFavorited ? 'fill-red-500 text-red-500' : 'text-surface-400')} />
            </button>
          )}
        </div>
        <p className="text-xs text-surface-500 dark:text-surface-400 flex items-center gap-1 mb-1.5">
...
// ─── Featured Listing Card (larger, homepage hero) ────────────────────────────
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4">
        {listing.featured && (
          <Badge variant="warning" className="mb-2"><Flame className="w-2.5 h-2.5" /> Featured</Badge>
        )}

...
export async function submitClaim(
  ...
  const { data, error } = await db()
    ...
    .insert({
      listing_id: listingId, listing_name: listingName, user_id: userId,
      user_email: userEmail, user_name: userName, phone, position, message,
      proof_documents: proofDocuments, status: 'pending',
    })
    .select('id').single();

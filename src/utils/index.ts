...
// ─── Date Helpers (Supabase returns ISO 8601 strings, not Firebase Timestamps) ─
...
export function getPriceLabel(range?: string | null): string {
...
export function getPriceSymbol(range?: string | null): string {
  const map: Record<string, string> = {
    budget:  '₦',
    mid:     '₦₦',
    premium: '₦₦₦',
...
export function isOpenNow(hours?: Record<string, { open: boolean; from: string; to: string }> | null): boolean | null {
